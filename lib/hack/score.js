...
export function scoreTarget(ns, host) {
  ...
  // Money-oriented baseline with a softened time penalty so long runtimes are less harshly punished.
  const hackTimeSeconds = hackTime / 1_000;
  const timePenalty = Math.pow(hackTimeSeconds, 0.75);

  return (maxMoney * serverGrowth * hackChance) / (minSec * timePenalty);
}
