...
function getBestHacknetAction(ns, config) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (levelCost > 0 && Number.isFinite(levelCost)) {
      const next = estimateNodeProduction(stats.level + 1, stats.ram, stats.cores, productionMult);
      const delta = Math.max(0, next - current);
      if (delta > 0) {
        const candidate = { type: "upgrade-level", node: i, cost: levelCost, delta, paybackSec: levelCost / delta };
    ...
    if (ramCost > 0 && Number.isFinite(ramCost)) {
      ...
      const delta = Math.max(0, next - current);
      if (delta > 0) {
        const candidate = { type: "upgrade-ram", node: i, cost: ramCost, delta, paybackSec: ramCost / delta };
        if (!best || candidate.paybackSec < best.paybackSec) best = candidate;
      }
    }

    const coreCost = ns.hacknet.getCoreUpgradeCost(i, 1);
    if (coreCost > 0 && Number.isFinite(coreCost)) {
      const next = estimateNodeProduction(stats.level, stats.ram, stats.cores + 1, productionMult);
      const delta = Math.max(0, next - current);
      if (delta > 0) {
        const candidate = { type: "upgrade-core", node: i, cost: coreCost, delta, paybackSec: coreCost / delta };
        if (!best || candidate.paybackSec < best.paybackSec) best = candidate;
      }
    }
  }
